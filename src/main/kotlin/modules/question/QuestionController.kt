...
class QuestionController(@Autowired private val questionRepository: QuestionRepository) {
    ...
    fun getAllQuestions(): List<Question> =
        questionRepository.findAll().toList()
    ...
    @GetMapping("/{id}")
    fun getQuestionById(@PathVariable("id") questionId: Long): ResponseEntity<Question> {
        val question = questionRepository.findById(questionId).orElse(null)
        return if (question != null) ResponseEntity(question, HttpStatus.OK)
        else ResponseEntity(HttpStatus.NOT_FOUND)
    }

    @PutMapping("/{id}")
    fun updateQuestionById(@PathVariable("id") questionId: Long, @RequestBody question: User): ResponseEntity<Question> {
    ...
    @DeleteMapping("/{id}")
    fun deleteQuestionById(@PathVariable("id") questionId: Long): ResponseEntity<Question> {
